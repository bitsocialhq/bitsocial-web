...
- Docs-site translations under `docs/i18n/**` have two helpers. `scripts/translate-docs.py` bulk-translates every page into every locale via Google Translate and has no per-file filter, so running it rewrites the whole corpus. `scripts/check-docs-translations.py` is the read-only verifier: it compares each locale copy against its English source and fails on translated code fences, dropped or rewritten links, altered inline code, changed `0x` contract addresses, heading/table/admonition drift, and files that were copied without being translated. Scope it with `--locales` and `--paths`, and treat a non-zero exit as a blocker. Advisory `frontmatter-untranslated` warnings are expected when a page title is a brand name.
